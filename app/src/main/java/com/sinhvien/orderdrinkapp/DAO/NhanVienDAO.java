...
public class NhanVienDAO {
    ...
    public boolean KiemTraEmail(String email) {
        String query = "SELECT * FROM " + CreateDatabase.TBL_NHANVIEN + " WHERE " + CreateDatabase.TBL_NHANVIEN_EMAIL + " = '" + email + "'";
        Cursor cursor = database.rawQuery(query, null);
    ...
    public boolean KiemTraPhone(String phone) {
        String query = "SELECT * FROM " + CreateDatabase.TBL_NHANVIEN + " WHERE " + CreateDatabase.TBL_NHANVIEN_SDT + " = '" + phone + "'";
        Cursor cursor = database.rawQuery(query, null);
        if (cursor.getCount() > 0) {
            return true;  // Email tồn tại
        }
    ...
    public boolean KiemTratk(String tentk) {
        String query = "SELECT * FROM " + CreateDatabase.TBL_NHANVIEN + " WHERE " + CreateDatabase.TBL_NHANVIEN_TENDN + " = '" + tentk + "'";
        Cursor cursor = database.rawQuery(query, null);
        if (cursor.getCount() > 0) {
            return true;  // Email tồn tại
        }
    ...
    public boolean CapNhatMatKhau(String email, String newPassword) {
        ContentValues contentValues = new ContentValues();
        contentValues.put(CreateDatabase.TBL_NHANVIEN_MATKHAU, newPassword);

        // Cập nhật mật khẩu dựa trên email
        long result = database.update(CreateDatabase.TBL_NHANVIEN, contentValues,
                CreateDatabase.TBL_NHANVIEN_EMAIL + " = ?",
                new String[]{email});
        return result > 0;
    }
